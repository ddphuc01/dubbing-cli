...
try:
    ...
    logger.warning("groq not installed. Install it using: pip install groq")
...
class TranslationService:
    ...
    def translate_with_retry(self, text, target_language="vi", max_retries=3, delay=1, service_priority=None):
        """
        ...
        if service_priority is None:
            service_priority = ["gemini", "groq", "openai", "qwen", "openrouter", "local", "gtx"]
            ...
            for service in service_priority:
                try:
                    ...
                    if translated_text:
                        ...
                        self.cache_manager.add_name(text.split()[0] if text.split() else "", cached_text.split()[0] if cached_text.split() else "")
                        return cached_text
                    else:
    ...
    def translate_subtitles(self, subtitles, target_language="vi", max_batch_size=500):
        """
    ...
    def translate_text_block(self, text_block, target_language="vi"):
        """
...
def translate_json_subtitles(input_file, output_file, target_language="vi"):
    """
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            translated_data = translation_service.translate_subtitles(data, target_language)
        elif isinstance(data, dict):
            # If it's a dictionary with subtitle data
            if 'segments' in data:
                translated_segments = translation_service.translate_subtitles(data['segments'], target_language)
                translated_data = {**data, 'segments': translated_segments}
            elif 'subtitles' in data:
                translated_subtitles = translation_service.translate_subtitles(data['subtitles'], target_language)
                ...
                for key, value in data.items():
                    if isinstance(value, list) and value and 'text' in value[0]:
                        translated_data[key] = translation_service.translate_subtitles(value, target_language)
                    else:
...
def main():
    ...
    parser.add_argument('output_file', help='Output translated subtitle file')
